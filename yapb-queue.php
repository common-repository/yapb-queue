...
function yapbq_add(){
	add_posts_page('YAPB queue','YAPB queue', 1, basename(__FILE__), 'yapbq_page');
}
		...
		if(isset($data['APP13'])){
			...
			if($iptc){
				...
				if($iptc["2#005"][0]) //title
					$updates['post_title'] = yapb_convert_charset($iptc["2#005"][0]);
				else if($iptc["2#105"][0]) //headline
					$updates['post_title'] = yapb_convert_charset($iptc["2#105"][0]);
